...
function sshAgent() {
    ...
    verbose "DEBUG: Ssh_Agent_Home=${Ssh_Agent_Home}" 28
    verbose "DEBUG: Ssh_Agent_State=${Ssh_Agent_State}" 28
    ...
    for Ssh_Dir in ${Ssh_Dirs[@]}; do
        if [ -r "${Ssh_Dir}/.ssh" ] && [ -d "${Ssh_Dir}/.ssh" ]; then
            ...
            done <<< "$(find "${Ssh_Dir}/.ssh/" -user ${BD_USER} -name "*id_ecdsa" -o -name "*id_rsa" -o -name "*ecdsa_key" -o -name "*id_ed25519" 2> /dev/null)"
        fi
    done
...
function sshAgentInit() {
    ...
    export Ssh_Agent_Home="${BD_HOME}/.ssh-agent"
    export Ssh_Agent_State="${Ssh_Agent_Home}.${BD_USER}@${HOSTNAME}"
    ...
    if [ "${BD_OS}" == "linux" ]; then
        ...
        done <<<"$(find /tmp/ssh* -type s -user ${BD_USER} -wholename "*/ssh*agent*" 2> /dev/null)"
    fi
    ...
    if [ ${#Ssh_Agent} -gt 0 ]; then
        [ "${BD_OS}" == "linux" ] && ssh_agent_pids=$(pgrep -u "${USER}" -f ${Ssh_Agent} -P 1 2> /dev/null)
        [ "${BD_OS}" == "darwin" ] && ssh_agent_pids=$(ps auxwww | grep "${USER}" | grep ${Ssh_Agent} | grep -v grep | awk '{print $2}')
        [ "${BD_OS}" == "windows" ] && ssh_agent_pids=$(ps | grep ${Ssh_Agent} | grep -v grep | awk '{print $2}')
    ...
    if [ "${USER}" == "${BD_USER}" ]; then
        if [ ${#SSH_AGENT_PID} -gt 0 ] && [ ${#SSH_AUTH_SOCK} -gt 0 ]; then
            verbose "DEBUG: creating ${Ssh_Agent_State}" 25
...
##
### check ssh, ssh-agent, & add all unique keys
##

Ssh_Agent_Clean_Counter=0

if [[ "${BD_KERNEL_NAME}" == *"cygwin"* ]]; then
    verbose "NOTICE: ${BASH_SOURCE} is not supported on Cygwin"
    return 1
fi
