...
if [ -z ${BD_HOME} ] || [ "${USER}" != 'root' ]; then
    [ -r ~/etc/bash.d/.bd/bd.sh ] && export BD_DEBUG=0 && export BD_ANSI_EXPORT=1 && source ~/etc/bash.d/.bd/bd.sh ${@}
else
    [ -r ${BD_HOME}/etc/bash.d/.bd/bd.sh ] && export BD_DEBUG=0 && export BD_ANSI_EXPORT=1 && source ${BD_HOME}/etc/bash.d/.bd/bd.sh ${@}
